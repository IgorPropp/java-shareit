...
public interface BookingStorage extends JpaRepository<Booking, Long> {
    ...
    List<Booking> findAllByBookerOrderByEndDesc(@Param("owner") Long owner);

    @Query("SELECT b FROM Booking b WHERE b.booker.id = :booker AND b.status = :status ORDER BY b.start DESC")
    List<Booking> findBookingByBookerAndStatusOrderByEndDesc(@Param("booker") Long booker, @Param("status") BookingStatus status);
    ...
    List<Booking> findBookingByOwnerAndStatusOrderByEndDesc(@Param("itemsId") List<Long> itemsId, @Param("status") BookingStatus status);
}
